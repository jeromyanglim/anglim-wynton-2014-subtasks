...
source('jags/jags_subtasks-2.r')
x <- model_recovery_subtaskspve(10)


x$fits[[1]]$dic$penalty

# getAnywhere('print.dic')
x$fits[[2]]$dic$penalty
...
source('jags/jags_subtasks-2.r')
x <- model_recovery_subtaskscus(5)
...
# have thinning: 10
posterior_predictive_check_cus <- function(strategies=FALSE, constraints=FALSE, n.iter=500) {
    jags_model <- jags_subtasks_gamma(f='power3', strategy_predictors=strategies, theta2_constraint=constraints, gamma_constraint=constraints)
    pc_model <- run_jags(jags_model$script, jagsdata,  jags_model$parameters, dic.run=FALSE, n.chains=1, n.iter=n.iter)
    pc_samples <- pc_model$samples[[1]]
...
returnfits_subtasks <- function(strategies=TRUE, constraints=TRUE) {
    jags_object <- run_jags(jags_subtasks_gamma(f='power3', strategy_predictors=strategies,
...
subtask_plot_by_model <- function(yhat_variable= 'yhat_sfcf', main='No Strategies; Unconstrained', ...) {
    yhat_group$subtask_order <- yhat_group$subtask_index
    yhat_group <- merge(yhat_group, meta.subtaskplot) # add meta data
    plot(y~trial, yhat_group, xlab = "Block", 
         ylab="Subtask completion time (sec)", ylim=c(0, 28), las=1, pch=yhat_group$pch, col=yhat_group$col,
         main=main, ...)
    axis(1, 1:15, 1:15)
    for (i in meta.subtaskplot$subtask) { 
      rbs_sub <- yhat_group[yhat_group$subtask==i, ]
      lines(rbs_sub$trial, rbs_sub[,yhat_variable], lty=rbs_sub$lty, col=rbs_sub$col)
    } 
}
par(mfrow=c(2,2))
subtask_plot_by_model('yhat_sfcf', 'No Strategies; Unconstrained')
subtask_plot_by_model('yhat_sfct', 'No Strategies; Constrained')
subtask_plot_by_model('yhat_stcf', 'Strategies; Unconstrained')
subtask_plot_by_model('yhat_stct', 'Strategies; Constrained')
...
# 
p <- ggplot(yhatdata_long[yhatdata_long$subject %in% 21, ], aes(trial, y, group=subtask)) + 
    geom_point(aes(colour = subtask, shape=subtask))
p <- p + geom_line(aes(x=trial, y=yhat, linetype=subtask))
p + facet_grid(subject ~ model)
