test_jags_overall <- function (f=c('power2', 'power3', 'exp3')) {
    f <- match.arg(f)
    x <- c(
"model {
    # Model
    for (i in 1:length(y)) {",
        if(f=="power2") {
"       mu[i] <- theta1[subject[i]] * pow(trial[i], theta2[subject[i]])"},
        if(f=="power3") {
"       mu[i] <- theta1[subject[i]] * pow(trial[i], theta2[subject[i]]) + theta3[subject[i]];"},
        if(f=="exp3") {
"       mu[i] <- theta1[subject[i]] * exp(theta2[subject[i]] * (trial[i] - 1)) + theta3[subject[i]];"},
...
cat(test_jags_overall(f='power3'))
